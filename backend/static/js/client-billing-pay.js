(function initClientBillingPay() {
  ...
  function loadPayPalSdk(clientId, currency) {
    ...
    sdkLoadPromise = new Promise((resolve, reject) => {
      ...
      script.onerror = () => reject(new Error("Le service PayPal est indisponible."));
  ...
  async function renderPayPalFundingButtons(paypal, form, config, container) {
    const root = form.closest("[data-client-billing-pay-root]");
    const sharedOptions = buildPayPalButtonOptions(form, config, root);
    ...
    for (const fundingSource of fundingSources) {
      ...
      host.className = "client-billing-pay-dialog__paypal-funding";
      host.dataset.paypalFunding = fundingSource;
  ...
  async function ensurePayPalButtons(form) {
    ...
    try {
      ...
      container.replaceChildren();
      const renderedCount = await renderPayPalFundingButtons(paypal, form, config, container);
