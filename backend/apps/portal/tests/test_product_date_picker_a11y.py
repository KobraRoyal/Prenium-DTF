...
class ProductDatePickerAccessibilityTests(SimpleTestCase):
    ...
    def test_calendar_runtime_manages_grid_focus_and_keyboard_navigation(self) -> None:
        ...
        self.assertIn("function portDatePopover", source)
        self.assertIn("function unportDatePopover", source)
        self.assertIn("function datePopoverHost", source)
        self.assertIn('node.closest("dialog")', source)
        self.assertIn("host.append(popover)", source)
        self.assertIn('classList.add("is-ported")', source)
        self.assertIn("isOutsideDatePicker(event, root, popover)", source)
        self.assertIn("product-date-picker:force-close", source)
