...
class StudioPolishUITests(SimpleTestCase):
    def test_studio_polish_follows_the_editor_component(self) -> None:
        ...
        billing_import = '@import "../components/client-billing-pay.css";'
        ...
        self.assertLess(entry.index(polish_import), entry.index(billing_import))
        self.assertLess(entry.index(billing_import), entry.index("@tailwind base"))
        self.assertIn("max-height: calc(min(44rem, 100dvh - 1.25rem) - 7.5rem) !important", entry)
        self.assertIn("max-height: min(44rem, calc(100dvh - 1.25rem)) !important", entry)
        ...
        self.assertIn(".gang-studio-checkout", polish)
        self.assertIn(".gang-studio-pay", polish)
        self.assertIn(".gang-studio-pay-dialog", polish)
        self.assertIn(".gang-studio-delivery__option", polish)
        self.assertIn("overflow-anchor: none", polish)
        ...
        self.assertIn(".gang-studio-checkout__dock", polish)
        self.assertIn(".gang-inspector-price-dock", polish)
        self.assertIn(".gang-editor.is-checkout [data-composition-only]", polish)
        self.assertIn(".gang-studio-order-meta", polish)
        self.assertIn("repeat(3, minmax(0, 1fr))", polish)
        self.assertIn("gang-studio-pay-dialog__provider", polish)
        self.assertIn("width: min(36rem, calc(100vw - 1.25rem)) !important", polish)
    ...
    def test_final_step_uses_one_confirm_cta(self) -> None:
        ...
        checkout_ui = checkout + dialog
        self.assertIn("studio_checkout.html", editor)
        self.assertIn("data-studio-checkout", checkout)
        self.assertIn("data-studio-checkout-form", checkout)
        self.assertIn("data-studio-checkout-open", checkout)
        self.assertIn("data-studio-pay-dialog", dialog)
        self.assertIn('closedby="none"', dialog)
        self.assertIn("b2b_shipping_choice.html", dialog)
        self.assertNotIn("b2b_shipping_choice.html", checkout)
        self.assertIn("shipping_house_number", dialog)
        self.assertIn("shipping_contact_name", dialog)
        self.assertIn("shipping_email", dialog)
        self.assertIn("Mode de livraison", dialog)
        self.assertIn("gang-studio-checkout__dock", checkout)
        self.assertIn("gang-inspector-price-dock", editor)
        self.assertIn("data-composition-only", editor)
        self.assertIn("is-checkout", editor)
        self.assertIn("data-inspector-panel-title", editor)
        self.assertIn('name="name"', checkout)
        self.assertIn('name="requested_date"', checkout)
        self.assertIn('name="customer_comment"', checkout)
        self.assertIn("Nom de la commande", checkout)
        self.assertIn("Date souhaitée", checkout)
        self.assertIn("Commentaire", checkout)
        self.assertNotIn("confirm_sheet_files", checkout)
        self.assertNotIn("Je confirme ce visuel", checkout)
        self.assertIn("delivery_destination", dialog)
        self.assertIn('type="hidden" name="delivery_destination"', dialog)
        self.assertNotIn("data-locked", dialog)
        self.assertNotIn("gang-studio-pay-dialog__eyebrow", dialog)
        self.assertNotIn(">Checkout<", dialog)
        self.assertIn("gang-studio-pay-dialog__provider", dialog)
        self.assertNotIn("gang-studio-pay--dialog", dialog)
        self.assertIn("studio_billing_complete", dialog)
        self.assertIn("studio_checkout_dialog.html", checkout)
        ...
        self.assertIn("portal:client-gang-sheet-checkout", checkout)
        self.assertIn("portal:client-gang-sheet-quote", editor)
        shipping = source(TEMPLATES_DIR / "portal/client/partials/b2b_shipping_choice.html")
        self.assertIn("b2b-shipping-choice__input", shipping)
        self.assertNotIn('class="ui-sr-only"', shipping)
        b2b_css = source(CSS_DIR / "components/b2b-order-project.css")
        self.assertIn(".b2b-shipping-choice__input {", b2b_css)
        self.assertIn("opacity: 0", b2b_css)
        self.assertNotIn("data-create-order-project", editor)
        self.assertNotIn("data-open-order-project", editor)
        self.assertNotIn("Continuer vers la commande", editor)
        self.assertNotIn("Continuer vers la commande", checkout)
        ...
        self.assertNotIn("Cochez la confirmation des visuels", editor_js)
        self.assertIn("quoteBox.hidden = studioCheckoutVisible()", editor_js)
        self.assertIn("function bindStudioShippingChoice()", editor_js)
        self.assertIn("function commitStudioChoiceAfterClick", editor_js)
