...
| UIUX-5.2 | Termine | `backend/templates/base.html`, `backend/templates/shop/services.html`, `backend/templates/portal/client/partials/checkout_uploads.html`, `backend/templates/portal/client/partials/checkout_summary.html`, `backend/templates/portal/staff/order_detail.html`, `backend/templates/portal/staff/panels/billing.html`, `backend/templates/portal/staff/panels/production.html`, `backend/templates/portal/staff/panels/scan.html`, `backend/templates/portal/staff/panels/shipping.html`, `backend/templates/components/portal/staff_meterage_section.html`, `backend/templates/prospects/step1.html`, `backend/templates/prospects/step2.html`, `backend/templates/prospects/step3.html`, `backend/templates/prospects/step4.html`, `backend/static_src/css/input.css`, `backend/static_src/css/components/landing.css`, `backend/static_src/css/components/product-shell.css`, `backend/static_src/css/components/prospect-tunnel.css`, `backend/static_src/css/components/shell.css`, `backend/static_src/css/components/workflow.css`, `backend/static_src/css/app.css`, `tests/ui/test_shop_checkout_ui.py`, `tests/ui/test_portal_ui.py`, `tests/ui/test_prospect_tunnel_ui.py`, `docs/sprints/sprint-uiux-frontend-premium.md` | Alignement global des vues restantes sur la landing brutaliste : page `/services/` reconstruite avec le langage `agency-*`, checkout et panneaux client/staff durcis dans le shell produit, feedback de soumission ajoute sur les formulaires HTMX, tunnel prospect carre et unifie avec boutons `ui-btn`, version statique portee a `20260613c`. | `npm run build:css` OK ; `DJANGO_SETTINGS_MODULE=config.settings.test DJANGO_SECRET_KEY=test-secret-key POSTGRES_PASSWORD=test-password python3 manage.py check` OK ; `DJANGO_SETTINGS_MODULE=config.settings.test DJANGO_SECRET_KEY=test-secret-key POSTGRES_PASSWORD=test-password pytest ../tests/ui/test_shop_checkout_ui.py ../tests/ui/test_portal_ui.py ../tests/ui/test_prospect_tunnel_ui.py ../tests/ui/test_portal_architecture.py` OK, 47 tests passes ; `docker compose config --quiet` OK ; `docker compose run --rm --no-deps web sh -lc 'cd /app/backend && python manage.py check'` OK ; `DJANGO_SETTINGS_MODULE=config.settings.test DJANGO_SECRET_KEY=test-secret-key POSTGRES_PASSWORD=test-password python3 manage.py collectstatic --noinput` OK ; navigateur integre OK sur `/services/`, `/client/`, checkout, tunnel prospect et menus mobiles marketing/portail, sans overflow horizontal. | Rationalisation du JS inline `order_tabs` reportee ; audit pixel-perfect multi-device non industrialise ; aucun changement backend metier ni permissions dans ce ticket. |
