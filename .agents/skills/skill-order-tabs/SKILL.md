...
- JS : `static_src/js/htmx/swap-state.js` (bundlé via `js/app.js`) — état actif basé sur le contrat sémantique `[role="tab"][data-panel-slug]`, confirmé avec le déclencheur `requestConfig.elt` de `htmx:afterSwap`, puis resynchronisé depuis `panel` sur `htmx:historyRestore`.
